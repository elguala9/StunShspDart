...
void main() {
  group('initializePointStunShsp', () {
    ...
    test('stunHandler is a StunHandlerBase', () {
      final handler = SingletonDIAccess.get<IStunShspHandler>();
      expect(handler.stunHandler, isA<StunHandlerBase>());
    });

    test('stunHandler.dualHandler is a DualStunHandler', () {
      final handler = SingletonDIAccess.get<IStunShspHandler>();
  ...
  group('migrate sockets', () {
    ...
    test('migrateSocketIpv4: socket port is exactly the new socket port', () async {
      // Bind a socket on a fixed port different from the original to guarantee
      // the ports cannot coincidentally match.
      final originalPort = handler.ipv4ShspSocket.localPort!;
      final fixedPort = (originalPort % 60000) + 2000; // deterministic, in valid range

      // Try up to a few ports in case fixedPort is already in use.
      ...
      // Fallback: port 0 — we just verify that localPort equals the new socket's port.
    ...
    test('migrateSocketIpv6: ipv6ShspSocket.localPort reflects new socket', () async {
      final hasIPv6 = await AddressUtility.canCreateIPv6Socket();
      if (!hasIPv6) {
        // ensure migrateSocketIpv6 installs the socket even if none existed
    ...
    test('migrateSocketIpv6: socket port is exactly the new socket port', () async {
      ...
      // Use a deterministic fixed port (distinct from original) if possible.
      ShspSocket? newSocket;
      int? chosenPort;
